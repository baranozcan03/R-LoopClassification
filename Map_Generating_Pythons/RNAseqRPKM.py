...
with open( intersectionFilePath , "r" ) as inputs:
     for line in inputs:
        ...
        StateAppear[index] = StateAppear[index] + int(line[4])
        stateLength[index] = stateLength[index] + int(line[2]) - int(line[1]) 
...
for i in range(0,10):
    rpkm = StateAppear[i] / ( ( stateLength[i] / 1000) * (readNum /1000000))
    rpkmF = "{:.2f}".format(rpkm)
    newline = "RNA-seq" + "," + "E" +str(i+1)  + "," + rpkmF + "\n"
...
with open( intersectionFilePath , "r" ) as inputs:
     for line in inputs:
        line = line.strip().split("\t")
        rpkm = int(line[4]) / ( ( 5000 / 1000) * (readNum /1000000))
